...
func _get_drag_data(_pos):
	if DataManager.skillData.data[get_parent().get_parent().select_index][get_index()]:
		var data = {};
		data["origin_node"] = icon;
		data["origin_panel"] = "SkillInventory";
		var skl:Dictionary = DataManager.skillData.data[get_parent().get_parent().select_index][get_index()];
...
func _can_drop_data(_pos, data):
	...
	if DataManager.skillData.data[get_parent().get_parent().select_index][target_inv_index] == null:
		data["target_item_id"] = null;
		data["target_texture"] = null;
		return true
	else:
		var skl = DataManager.skillData.data[get_parent().get_parent().select_index][target_inv_index];
...
#	var origin_slot = data["origin_node"].get_parent().name;
	...
	#更新原slot的数据
	if data["origin_panel"] == "SkillInventory":
		DataManager.skillData.data[get_parent().get_parent().select_index][origin_index] = data["target_item_id"];
	else:
		pass
	...
	#更新目标的纹理和数据
	if DataManager.skillData.data[get_parent().get_parent().select_index][target_inv_index] == null:
		DataManager.skillData.data[get_parent().get_parent().select_index][target_inv_index] = {};
	DataManager.skillData.data[get_parent().get_parent().select_index][target_inv_index] = data["origin_item_id"];
#	PlayerData.inv_data[target_inv_index]["Item"] = data["origin_item_id"]
...
#		var slot = data["origin_node"].get_parent().get_parent().get_child(origin_index);
#		if origin_visible == true:
#			slot.get_node("lv").text = str(data["target_item_id"]["show_lv"]);
#		slot.get_node("lv").visible = origin_visible;
#		slot.get_node("static_lv").visible = origin_visible;
#	else:
#		var slot = data["origin_node"].get_parent().get_parent().get_child(origin_index);
#		if origin_visible == true:
#			slot.lv.text = str(data["target_item_id"]["show_lv"]);
...
func _on_SkillSlot_mouse_entered():
	select.visible = true;
	var skl = DataManager.skillData.data[get_parent().get_parent().select_index][get_index()];
	if not skl == null:
		GlobalManager.main.on_show_toolTipSkill(skl.id,skl.show_lv,get_local_mouse_position());


func _on_SkillSlot_mouse_exited():
