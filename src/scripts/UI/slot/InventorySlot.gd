...
func _get_drag_data(_pos):
	if DataManager.invData.data[get_parent().get_parent().select_index][get_index()]:
#	if not PlayerData.inv_data[get_parent().get_index()].empty():
		var data = {};
		data["origin_node"] = icon;
		data["origin_panel"] = "Inventory";
		var item = DataManager.invData.data[get_parent().get_parent().select_index][get_index()];
...
func _can_drop_data(_pos, data):
	...
	if DataManager.invData.data[get_parent().get_parent().select_index][target_inv_index] == null:
		data["target_item_id"] = null;
		data["target_texture"] = null;
		return true
	else:
		var item = DataManager.invData.data[get_parent().get_parent().select_index][target_inv_index];
...
#	var target_inv_slot = name;
	...
	if data["origin_panel"] == "Inventory":
		...
		DataManager.invShortcurData.data[origin_index] = data["target_item_id"];
	else:
	...
	#更新目标的纹理和数据
	DataManager.invData.data[get_parent().get_parent().select_index][target_inv_index] = data["origin_item_id"];
...
func _on_ItemSlot_mouse_entered():
	...
	if DataManager.invData.data[get_parent().get_parent().select_index][get_index()] != null:
		GlobalManager.main.on_show_toolTipEquip("Inventory","",get_index(),get_local_mouse_position());
