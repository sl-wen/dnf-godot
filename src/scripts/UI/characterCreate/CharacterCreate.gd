...
@onready var cut_swordman:Sprite2D = $cutscene_swordman;
@onready var cut_fighter:Sprite2D = $cutscene_fighter;
@onready var cut_gunner:Sprite2D = $cutscene_gunner;
@onready var cut_mage:Sprite2D = $cutscene_mage;
@onready var cut_priest:Sprite2D = $cutscene_priest;
...
@onready var job_title:Sprite2D = $job_title;
@onready var job_hard:Sprite2D = $job_hard;
@onready var job_des:Sprite2D = $job_des;
...
func _ready():
	swordmanBtn.button_pressed = true;
	createWindow.visible = false;
	@warning_ignore("return_value_discarded")
	ArchiveManager.connect("role_create_ok", Callable(self, "_on_role_create_ok"));
...
func change_cutscene(value1:bool,value2:bool,value3:bool,value4:bool,value5:bool):
	...
	tween = create_tween()
	if value1 == true:
		tween.tween_property(cut_swordman, "position", Vector2(257, 241), 0.4).from(Vector2(-100, 241))
		tween.set_trans(Tween.TRANS_QUART).set_ease(Tween.EASE_OUT)
	elif value2 == true:
		tween.tween_property(cut_fighter, "position", Vector2(132, 299), 0.4).from(Vector2(-200, 299))
		tween.set_trans(Tween.TRANS_QUART).set_ease(Tween.EASE_OUT)
	elif value3 == true:
		tween.tween_property(cut_gunner, "position", Vector2(210, 300), 0.4).from(Vector2(-100, 300))
		tween.set_trans(Tween.TRANS_QUART).set_ease(Tween.EASE_OUT)
	elif value4 == true:
		tween.tween_property(cut_mage, "position", Vector2(130, 215), 0.4).from(Vector2(-200, 215))
		tween.set_trans(Tween.TRANS_QUART).set_ease(Tween.EASE_OUT)
	elif value5 == true:
		tween.tween_property(cut_priest, "position", Vector2(162, 313), 0.4).from(Vector2(-200, 313))
		tween.set_trans(Tween.TRANS_QUART).set_ease(Tween.EASE_OUT)
